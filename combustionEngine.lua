...
local function checkHydroLocking(device, dt)
    ...
    local dryRate = -0.02 -- 2% per second when drying (slower than flooding)
...
local function updateGFX(device, dt)
    ...
    if device.starterEngagedCoef > 0 then
        ...
        if not device.starterThrottleKillTimerStart or device.starterThrottleKillTimerStart == 0 then
            device.starterThrottleKillTimerStart = 2.0 -- 2 second hesitation period
        ...
        local rpmRatio = device.outputAV1 /
        ...
        if device.outputAV1 > device.idleAV * 1.1 then
            ...
            device.starterThrottleKillCoefSmoother:set(
                device.starterThrottleKillCoef)
            device.starterDisabled = false
    ...
    -- At idle (800): ~0.76
    -- At max RPM: 0.8
    local baseBrightness = linearScale(currentRPM, 0, device.maxRPM, 0.6, 1.0) *
    ...
    electrics.values.electricalLoadCoef = math.min(
                                              math.max(brightness *
                                                           batteryLoadFactor,
                                                       0.3), 1.0)
    ...
    if device.shutOffSoundRequested and device.outputAV1 < device.idleAV * 0.95 and
        device.outputAV1 > device.idleAV * 0.5 then
    ...
    if not device.isDisabled and (frictionTorque > device.maxTorque or
        (device.outputAV1 < device.idleAV * 0.5 and frictionTorque > idleTorque *
            0.95)) then
...
local function updateTorque(device, dt)
    ...
    local engineTempC = (device.thermals and
                            device.thermals.engineBlockTemperature or 293.15) -
                            273.15 -- Default to 20°C if not available
    ...
    torque = min((torque * device.forcedInductionCoef * throttleMap) +
                     device.nitrousOxideTorque) * device.outputTorqueState *
                 (ignitionCut and 0 or 1) * device.slowIgnitionErrorCoef *
                 device.fastIgnitionErrorCoef * device.starterIgnitionErrorCoef
    ...
    if device.starterTorque then
        ...
        local isDiesel = (device.requiredEnergyType == "diesel") or
                             (device.engineType and
                                 (device.engineType == "diesel" or
                                     device.engineType == "dieselElectric"))
    ...
    baseStarterTorque = baseStarterTorque * (1 - floodTorqueReduction) *
                            (1 - tempTorqueReduction) * wearFactor
    ...
    local minFuelForCombustion = isCranking and (0.01 * tempAdjustment) or
                                     (0.1 * tempAdjustment)
    local minAirForCombustion = isCranking and (0.04 * tempAdjustment) or
                                    (0.4 * tempAdjustment)
    ...
    elseif device.outputAV1 > device.idleAV or (device.starterMaxAV or 100) *
        1.1 then
    ...
    -- Update electrics.volts with the smoothed gauge voltage
    electrics.values.volts = smoothedVoltage
    ...
    if isCranking and not isEngineRunning then
        ...
        -- Phase 0: Initial crank (first 0.3 seconds)
        if device.startingHesitationPhase == 0 then
            if device.startingHesitationTime > 0.3 then
            ...
            -- Phase 1: Struggle phase (1-2 seconds)
        elseif device.startingHesitationPhase == 1 then
            local struggleDuration = 1.0 + math.random() -- Random struggle duration between 1-2 seconds

            -- Create a pulsing effect during struggle
            local pulseFreq = 3.0 -- Hz
            local pulse = (math.sin(
                              device.startingHesitationTime * math.pi * 6 *
                                  pulseFreq) + 1) * 0.5 -- 0-1 pulse

            -- Base hesitation effect (70-100% of normal torque)
            device.startingHesitationFactor = 0.7 + pulse * 0.3
            log('D', 'startingHesitation',
            ...
            if math.random() < 0.1 then
                device.startingHesitationFactor =
                    device.startingHesitationFactor *
                        (0.3 + math.random() * 0.4) -- 30-70% of current factor
                log('D', 'startingHesitation',
    ...
    if isDiesel then
        ...
        local baseTorque = device.starterTorque * 1.2 or device.starterMaxAV *
                               2.95 -- Increased base torque for diesel

        -- More aggressive battery voltage scaling - diesels need more power
        local voltageEffect = math.pow(batteryVoltageFactor, 0.8) -- More sensitive to voltage drops
        ...
        maxTorqueAtZeroRPM = baseTorque *
                                 (1 +
                                     (combinedOscFactor * 0.2 *
                                         compressionEffect))

        -- Minimum torque at high RPM - ensure engine keeps spinning
        minTorqueAtHighRPM = device.invStarterMaxAV * 0.128 * voltageEffect *
                                 tempEffect
    else
        -- Gasoline engines - less compression, easier starting
        local baseTorque = device.starterTorque * 1.45 or device.starterMaxAV *
                               18.26 -- Increased base torque for better cranking

        -- Battery voltage scaling
        local voltageEffect = math.pow(batteryVoltageFactor, 0.8)
        baseTorque = baseTorque * (0.02 + 0.95 * voltageEffect)

        -- Temperature effect - still significant but less than diesel
        local normalizedTemp =
            math.max(0, math.min(1, (engineTempC + 20) / 100)) -- Normalize -20°C to 80°C to 0-1 range
        ...
        maxTorqueAtZeroRPM = baseTorque *
                                 (1 +
                                     (combinedOscFactor * 0.4 *
                                         compressionEffect))

        -- Minimum torque at high RPM
        minTorqueAtHighRPM = device.invStarterMaxAV * 0.076 * voltageEffect *
                                 tempEffect
    end
    ...
    local engineSpeedFactor = math.max(0, 0.9 - math.abs(engineAV) *
                                           device.invStarterMaxAV)
    local baseStarterTorque = device.starterEngagedCoef * maxTorqueAtZeroRPM *
                                  engineSpeedFactor
    ...
    if device.starterEngagedCoef > 0 then
        ...
        local cycleAdvance =
            (math.abs(device.outputAV1) * dt) / radiansPerCycle * cylinderCount
        ...
        local currentCylinder = math.floor(
                                    (device.cyclePosition % (2 * math.pi)) /
                                        cyclePosPerCylinder) + 1
        ...
        for i = 1, cylinderCount do
            ...
            local cyclePos = (device.cyclePosition - cylinderAngle) %
                                 (2 * math.pi)
            ...
            if stroke == 0 and not cylinder.isFiring and throttle > 0.1 then
                ...
                local isReadyForFuel = timeSinceLastFuel >
                                           (isCranking and 0.3 or 0.2) -- More frequent injection when cranking
                ...
                if (isStarting or isRunning) and isReadyForFuel then
                    ...
                    if cylinderFlood > 0.7 then
                        ...
                        if math.random() < 0.1 then
                            device.cylinderFloodLevels[i] = math.max(0,
                                                                     cylinderFlood -
                                                                         0.1)
                        end
                    ...
                    if debugFuel and fuelAmount > 0 then
                        ...
                        if currentTime - device.lastFuelLogTime[i] > 1.0 then -- Limit to once per second per cylinder
                            print(string.format(
                                      ...
                                      device.temperature or 0, currentTime))
            ...
            if cylinder.isFiring then
                ...
                if hasEnoughFuel and hasEnoughAir and hasEnoughIgnition then
                    ...
                    local tempFactor = device.thermals and
                                           (device.thermals
                                               .engineBlockTemperature / 100) or
                                           0.5
                    ...
                    local combustionEfficiency =
                        math.min(cylinder.fuelAmount, cylinder.airAmount) *
                            (0.7 + tempFactor * 0.3)
                    ...
                    if debugFuel then
                        ...
                        if currentTime - device.lastCombustLogTime[i] > 0.5 then -- Limit to twice per second per cylinder
                            print(string.format(
                                      ...
                                      cylinder.airAmount, cylinder.temperature,
                                      currentTime))
                    ...
                    if debugFuel then
                        ...
                        if currentTime - device.lastMisfireLogTime[i] > 0.5 then -- Limit to twice per second per cylinder
                            print(string.format(
                                      ...
                                      minIgnitionForCombustion,
                                      device.temperature or 0, isCranking and
                                          "CRANKING" or
                                          (isRunning and "RUNNING" or "STARTING"),
                                      throttle, math.abs(device.outputAV1) *
                    ...
                    if cylinder.fuelAmount > minFuelForCombustion * 0.1 and
                        cylinder.misfireCount > 1 then -- After 2 consecutive misfires with fuel
                        ...
                        if isCranking and device.floodLevel < 1.0 and
                            cylinder.misfireCount > 5 and cylinder.fuelAmount >
                            minFuelForCombustion then
                            local floodIncrement = 0.005 *
                                                       (1.0 +
                                                           cylinder.fuelAmount *
                                                           0.005) -- 0.5% to 1.5% increase based on fuel
                            device.floodLevel = math.min(1.0,
                                                         (device.floodLevel or 0) +
                                                             floodIncrement)
                            if debugFuel then
                                ...
                                if currentTime - device.lastFloodIncLogTime[i] >
                                    0.5 then -- Limit to twice per second per cylinder
                                    print(string.format(
        ...
        device.cyclePosition = (device.cyclePosition + cycleAdvance) %
                                   (cylinderCount * 2)
        ...
        local engineBlockTemp = device.thermals and
                                    device.thermals.engineBlockTemperature or 20 -- Default to room temp if not available
        ...
        if strokePhase == 0 then
            ...
            local coldPowerReduction = math.max(0.3, 1.0 -
                                                    ((60 -
                                                        math.min(60, engineTempC)) /
                                                        60) * 0.7) -- Down to 30% power when very cold
            strokeEffect = strokeAmplitude * (phaseSine - 1) * powerStrength *
                               coldPowerReduction
        end
        ...
        local modifiedTorque = max(baseStarterTorque * (1 - strokeEffect),
                                   minStarterTorque)

        -- Smooth the transition between strokes
        device.lastModifiedTorque = device.lastModifiedTorque or
                                        baseStarterTorque
        starterTorque = device.lastModifiedTorque +
                            (modifiedTorque - device.lastModifiedTorque) * 0.9 -- 20% smoothing
...
local function activateStarter(device)
    ...
    if device.starterEngagedCoef ~= 1 then
        ...
        local maxCrankingTime = device.requiredEnergyType == "diesel" and 10 or
                                    5
        ...
        elseif engineTempC < 260 and engineTempC > -268 and device.hasFuel then -- Changed from -45°C - 15°C to < 260°C - >-268°C for more realistic warm-up and added realism to the engine starting process
            ...
            local tempFactor = linearScale(engineTempC, -270, 260, 2.0, 0.2) -- Scale from -30°C to 60°C
            ...
            local errorFactor = linearScale(engineTempC, -270, 260, 1.0, 0.1)
            device.starterIgnitionErrorTimer =
                linearScale(engineTempC, -270, 260, 2.8, 0.2)
            device.starterIgnitionErrorChance =
                linearScale(engineTempC, -270, 260, 0.7, 0.05) * errorFactor
            device.starterIgnitionErrorCoef =
                linearScale(engineTempC, -270, 260, 0.8, 0.1) * errorFactor

            -- Adjust idle RPM fluctuations (less when warm)
            device.idleAVReadError = linearScale(engineTempC, -270, 260, 0.3,
                                                 0.05)
            device.idleAVReadErrorChance =
                linearScale(engineTempC, -270, 260, 0.6, 0.1)
...
local function new(jbeamData)
    ...
    device.fundamentalFrequencyCylinderCount =
        jbeamData.fundamentalFrequencyCylinderCount or 8
